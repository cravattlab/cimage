...
print OUTFILE "<A HREF=\"http://137.131.5.31/cgi-bin/chuquest/batch_annotate.pl?dset=$cwd/$intable\">Gabe's Uniprot Batch Annotation</A><BR><BR>\n";
...
for ($i=0; $i<$ncol; ++$i) {
    ...
    print OUTFILE "<TH bgcolor=\"HoneyDew\"><A HREF=\"http://137.131.5.31/cgi-bin/chuquest/cimage_sort_table.pl?dset=$dset&colname=$header[$i]&ascending=True\" style=\"text-decoration:none\">^</A> <A HREF=\"http://137.131.5.31/cgi-bin/chuquest/cimage_sort_table.pl?dset=$dset&colname=$header[$i]&ascending=False\" style=\"text-decoration:none\">v</A>\n";
}
}
print OUTFILE "<TBODY>\n";
for ($i=1; $i<$nrow; ++$i) {
