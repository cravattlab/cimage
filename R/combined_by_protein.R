...
for (uniq in levels(as.factor(entries) ) ) {
  ...
  for ( k in 1:nset ) {
    ...
    if (sum(pass2) >= 1 ) {
      ...
      if (sum(pass2) == 2){
        if( (max(sub.table[pass2,vn1[k]]) == 20.00) & (min(sub.table[pass2,vn1[k]]) < 3.00) ){
           out.num.matrix[count,k] <- round(min(sub.table[pass2,vn1[k]],na.rm=T),digits=2)
        }
      }
...
yl <- c(0, max(ratio))
for ( i in 1:nset) {
  plot(x,ratio[,i],ylim=yl,xlab="Peptide Count",ylab="Observed Ratio",col=palette()[i])
  par(new=T)
}
par(new=F)
