...
## file name tags
cross.vec <- dimnames(cross.table)[[2]][-c(1:3)]
...
for ( i in 1:dim(cross.table)[1] ) {
  peptide <- cross.table[i,"peptide"]
  ipi <- cross.table[i,"ipi"]
  entry.tag <- paste( ipi, peptide, sep=":")
  entry.index <- which( entry.tag == entry.levels )
  description <- as.character(original.table[as.character(cross.table[i,"ipi"]),1])
  symbol<- strsplit(description, " ")[[1]][1]
  nmod <- length(strsplit(as.character(peptide),"*",fixed=T)[[1]])-1
  mono.mass  <- cross.table[i,"mass"] <- cross.table[i,"mass"]+nmod*probe.mass
  ...
  for( j in 1:length(cross.vec) ) {
    scan.str <- as.character(cross.table[i,cross.vec[j]])
    if (scan.str == "none") {
      raw.file.charge[j] <- "none.none"
      raw.scan.num[j] <- NA
    } else {
      tmp.vec <- unlist( strsplit(scan.str, "\\."))
...
##             ylim=range(log10(best.light.yes),log10(best.heavy.yes)))
    ...
    this.df <- data.frame(index=i, ipi=ipi, description=description, symbol=symbol, sequence=peptide,
                          mass=round(mass,digits=4), charge=charge, segment=raw.index,
                          ...
                          entry=entry.index,link=paste('=HYPERLINK(\"./PNG/',out.filename.base,'-',as.character(all.count-1),'.png\")',sep=''))
